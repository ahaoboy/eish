...
Each one opens with a header saying it is generated and should not be edited,
plus the exact command that produced it — so a file found in someone's CI can be
traced back and regenerated:
...
#
# This file was generated by eish 0.1.0 and is not meant to be
...
#
#   eish quickjs-ng/quickjs@v0.16.2 --shell bash --proxy xget
#
# Run `./install.sh --help` for the available options.
```

The command is the real one, taken from the arguments `eish` was called with,
not a summary — it includes things the script itself has no trace of, such as
`--release-json`. Values of secret-bearing options are replaced with
`<redacted>`, so a token passed on the command line never ends up in a file
